...
function RouteComponent() {
  ...
  const deleteEmployeeMutation = useMutation({
    mutationFn: async (id: string) => {
      const { client } = await import("@/utils/orpc");
      return client.payroll.employees.delete({ id });
    },
    onSuccess: () => {
  ...
  const employees: Employee[] = useMemo(() => {
    ...
    return apiEmployees.map((emp: any) => {
      ...
      // Simple estimation for UI display - Real calc should use the calculate endpoint
      ...
      return {
        id: emp.id,
        employeeId: `GKN-${emp.id.slice(0, 4).toUpperCase()}`, // Generate a visual ID
        firstName: nameParts[0] || "Unknown",
        lastName: nameParts.slice(1).join(" ") || "",
        email: emp.email || "",
        phone: "+592-000-0000", // Placeholder until added to schema
        ...
        grossSalary,
        basicSalary: grossSalary * 0.8, // Estimation
        allowances: grossSalary * 0.2, // Estimation
        deductions,
        netSalary,
        payeRate: 0.175,
        payeAmount,
        nisContribution,
        bankAccount: emp.bankAccount || "ACC-XXXXXXXXX",
  ...
  // Filter logic is now handled by the API query mostly, but we keep this for client-side responsiveness if needed
  // Note: The query above already passes filters to the API.
  // The filteredEmployees below effectively just returns 'employees' unless we want to do extra client-side filtering.
      ...
      <header className="mb-8">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button onClick={() => navigate({ to: "/payroll/employees/new" })}>

...
export const serviceCatalogRouter = {
    ...
    list: protectedProcedure
      ...
      .handler(async ({ input, context }) => {
        ...
        if (filters.isFeatured !== undefined) {
          conditions.push(
            eq(
              serviceCatalogSchema.serviceCatalog.isFeatured,
              filters.isFeatured
            )
          );
        }
  ...
  milestones: {
    list: protectedProcedure
      ...
      .handler(async ({ input, context }) => {
        ...
        const conditions = [
          eq(
            serviceCatalogSchema.projectMilestones.projectId,
            filters.projectId
          ),
        ];
  ...
  timeEntries: {
    list: protectedProcedure
      ...
      .handler(async ({ input, context }) => {
        ...
        if (filters.milestoneId) {
          conditions.push(
            eq(
              serviceCatalogSchema.timeEntries.milestoneId,
              filters.milestoneId
            )
          );
        }
        ...
        if (filters.dateFrom) {
          conditions.push(
            gte(
              serviceCatalogSchema.timeEntries.date,
              new Date(filters.dateFrom)
            )
          );
        }
    ...
    markAsBilled: protectedProcedure
      .use(requirePermission("timeEntries.update"))
      ...
      .handler(async ({ input, context }) => {
        ...
        await db
          ...
          .where(
            and(
              eq(serviceCatalogSchema.timeEntries.isBilled, false)
              // Using individual updates for each ID
            )
          );

        // Update each entry individually since inArray might have compatibility issues
    ...
    delete: protectedProcedure
      ...
      .handler(async ({ input, context }) => {
        ...
        await db
          ...
          .where(
            eq(serviceCatalogSchema.serviceDocumentTemplates.id, input.id)
          );
  ...
  // ========================================
  // CLIENT COMMUNICATIONS SUB-ROUTER
  // ========================================
  communications: {
    list: protectedProcedure
      ...
      .handler(async ({ input, context }) => {
        ...
        const [items, totalResult] = await Promise.all([
          db
            ...
            .offset(offset)
            .orderBy(
              desc(serviceCatalogSchema.clientCommunicationLog.createdAt)
            ),
          db
            .select({ count: count() })
            .from(serviceCatalogSchema.clientCommunicationLog)

...
export const backupRouter = {
        ...
        switch (input.backupType) {
          ...
          case "SELECTIVE":
            if (!input.includeTables?.length) {
              throw new ORPCError(
                "BAD_REQUEST",
                "Selective backup requires includeTables"
              );
            }
  ...
  getById: protectedProcedure
    ...
    .handler(async ({ input, context }) => {
      ...
      if (!backup.length) {
        throw new ORPCError("NOT_FOUND", "Backup not found");
      }
  ...
  verify: protectedProcedure
    ...
    .handler(async ({ input, context }) => {
      ...
      if (!backup[0].storagePath) {
        throw new ORPCError("BAD_REQUEST", "Backup file path not available");
      }

      const backupService = getBackupService();
      const result = await backupService.verifyBackup(backup[0].storagePath);
    ...
    .handler(async ({ input, context }) => {
      ...
      try {
        ...
        await db.insert(backupSchema.restoreOperations).values({
          ...
          name: input.name || `Restore from ${backup[0].name}`,
          description: input.description,
          status: "IN_PROGRESS",
  ...
  updateSchedule: protectedProcedure
    ...
    .handler(async ({ input, context }) => {
      ...
      if (!existing.length) {
        throw new ORPCError("NOT_FOUND", "Schedule not found");
      }
      ...
      const [updated] = await db
        ...
        .set({
          ...
          nextRunAt: updates.cronExpression
            ? calculateNextRun(updates.cronExpression, existing[0].timezone)
            : undefined,
        })
        .where(eq(backupSchema.backupSchedules.id, id))
        .returning();
  ...
  listRestoreOperations: protectedProcedure
    .use(requirePermission("system.admin"))
    .input(
      z.object({
        page: z.number().int().min(1).default(1),

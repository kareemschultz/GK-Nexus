...
export const ocrRouter = {
    ...
    .handler(async ({ input, context }) => {
      ...
      try {
        ...
        if (!supportedMimeTypes.includes(document.mimeType)) {
          throw new ORPCError(
            "BAD_REQUEST",
            "Document type not supported for OCR processing"
          );
        }
        ...
        throw new ORPCError(
          ...
          "Failed to submit document for OCR processing"
        );
      }
    }),
  ...
  getProcessingStatus: protectedProcedure
    ...
    .handler(async ({ input, context }) => {
      ...
      try {
        ...
        if (!ocrJob) {
          throw new ORPCError("NOT_FOUND", "OCR processing job not found");
        }
        ...
        throw new ORPCError(
          "INTERNAL_SERVER_ERROR",
          "Failed to get OCR processing status"
        );
      }
    }),
  ...
  getExtractedData: protectedProcedure
    ...
    .handler(async ({ input, context }) => {
      ...
      try {
        ...
        if (!ocrResult) {
          throw new ORPCError("NOT_FOUND", "OCR results not found");
        }
  ...
  validateResults: protectedProcedure
    ...
    .handler(async ({ input, context }) => {
      ...
      try {
        ...
        if (!ocrResult) {
          throw new ORPCError("NOT_FOUND", "OCR result not found");
        }
        ...
        await db
          ...
          .set({
            status: "VALIDATED",
            confidenceScore:
              corrections.confidence || ocrResult.confidenceScore,
          })
          .where(eq(businessSchema.ocrProcessingJob.id, processingId));
        ...
        throw new ORPCError(
          "INTERNAL_SERVER_ERROR",
          "Failed to validate OCR results"
        );
      }
    }),
  ...
  getProcessingStats: protectedProcedure
    ...
    .handler(async ({ input, context }) => {
      ...
      try {
        ...
        if (clientId) {
          conditions.push(
            eq(businessSchema.ocrProcessingJob.clientId, clientId)
          );
        }
  ...
  batchProcess: protectedProcedure
    ...
    .handler(async ({ input, context }) => {
      ...
      try {
        ...
        throw new ORPCError(
          "INTERNAL_SERVER_ERROR",
          "Failed to initiate batch processing"
        );
      }
    }),
    ...
    .handler(async ({ input, context }) => {
      ...
      try {
        ...
        if (batchJobs.length === 0) {
          throw new ORPCError("NOT_FOUND", "Batch not found");
        }
        ...
        const avgConfidence =
          batchJobs
            .filter((job) => job.confidenceScore !== null)
            .reduce((sum, job) => sum + (job.confidenceScore || 0), 0) /
          Math.max(
            1,
            batchJobs.filter((job) => job.confidenceScore !== null).length
          );
  ...
  // Helper method to simulate OCR processing (would be replaced with actual OCR service)

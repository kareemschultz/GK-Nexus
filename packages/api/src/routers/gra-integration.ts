...
export const graAuthenticate = protectedProcedure
  ...
  .handler(async ({ input, context }) => {
    ...
    const { clientId, tin, username, password: _password } = input;
    ...
    try {
      ...
      // Note: This is a simplified implementation. In production, use proper encryption
      // and match the actual graApiCredential schema (organizationId, apiKey, etc.)
      const credentialId = `gra_cred_${clientId}`;
      await db
        ...
        .values({
          ...
          apiSecret: tin,
          environment: "sandbox",
          description: `GRA credentials for ${username}`,
          isActive: true,
          expiresAt: sessionExpiry,
          createdBy: user?.id!,
        })
        .onConflictDoUpdate({
          target: [graIntegrationSchema.graApiCredential.id],
          set: {
            apiKey: authToken,
            expiresAt: sessionExpiry,
            lastUsedAt: new Date(),
      ...
      throw new ORPCError("INTERNAL_SERVER_ERROR", {
        message: "Failed to authenticate with GRA eServices",
      });
    }
  });
    ...
    try {
      ...
      const [credentials] = await db
        ...
        .where(
          and(
            eq(
              graIntegrationSchema.graApiCredential.id,
              `gra_cred_${clientId}`
            ),
      ...
      if (!credentials) {
        throw new ORPCError("UNAUTHORIZED", {
          message: "GRA authentication required. Please authenticate first.",
        });
      }
      ...
      await db.insert(graIntegrationSchema.graApiSync).values({
        id: `gra_sync_${Date.now()}`,
        organizationId: credentials.organizationId,
        syncType,
        status: "completed",
        lastSyncAt: new Date(),
        recordsProcessed: 1,
        metadata: JSON.stringify(syncData),
      });
      ...
      throw new ORPCError("INTERNAL_SERVER_ERROR", {
        message: `Failed to sync ${input.syncType.toLowerCase()} with GRA`,
      });
    }
  });
...
export const graSubmitFiling = protectedProcedure
  ...
  .handler(async ({ input, context }) => {
    ...
    try {
      ...
      if (!credentials) {
        throw new ORPCError("UNAUTHORIZED", {
          message: "GRA authentication required for this client",
        });
      }
      ...
      await db.insert(graIntegrationSchema.activityLog).values({
        ...
        description: `Submitted ${formType} filing to GRA`,
      ...
      throw new ORPCError("INTERNAL_SERVER_ERROR", {
        message: "Failed to submit filing to GRA",
      });
    }
  });
...
export const graCheckSubmissionStatus = protectedProcedure
  ...
  .handler(async ({ input, context }) => {
    ...
    try {
      ...
      throw new ORPCError("INTERNAL_SERVER_ERROR", {
        message: "Failed to check GRA submission status",
      });
    }
  });
...
export const graGetFilingCalendar = protectedProcedure
  ...
  .handler(async ({ input }) => {
    const { year, clientId: _clientId } = input;
...
export const graExportClientData = protectedProcedure
  ...
  .handler(async ({ input, context }) => {
    ...
    try {
      ...
      for (const clientId of clientIds) {
        ...
        const [client] = await db
          .select({
            ...
            tin: businessSchema.client.tinNumber,
        ...
        switch (exportType) {
          case "ANNUAL_SUMMARY": {
            ...
            const annualSummary = await db
              .select({
                totalRevenue:
                  sql<number>`COALESCE(SUM(${businessSchema.invoice.total}), 0)`.as(
                    "totalRevenue"
                  ),
                totalVAT:
                  sql<number>`COALESCE(SUM(${businessSchema.invoice.vatAmount}), 0)`.as(
                    "totalVAT"
                  ),
              })
            ...
            const payrollSummary = await db
              .select({
                totalPayroll:
                  sql<number>`COALESCE(SUM(${businessSchema.payrollRecord.grossSalary}), 0)`.as(
                    "totalPayroll"
                  ),
                totalPAYE:
                  sql<number>`COALESCE(SUM(${businessSchema.payrollRecord.payeTax}), 0)`.as(
                    "totalPAYE"
                  ),
                totalNIS:
                  sql<number>`COALESCE(SUM(${businessSchema.payrollRecord.nisEmployee} + ${businessSchema.payrollRecord.nisEmployer}), 0)`.as(
                    "totalNIS"
                  ),
              })
      ...
      throw new ORPCError("INTERNAL_SERVER_ERROR", {
        message: "Failed to export client data for GRA",
      });
    }
  });
